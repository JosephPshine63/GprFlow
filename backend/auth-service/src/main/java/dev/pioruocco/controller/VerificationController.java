...
import dev.pioruocco.request.ResetPasswordRequest;
...
/**
 * Holds the credential/OTP-verification endpoints that used to live on the monolith's
 * UserController (2FA activation, email verification, password reset) — everything here is
 * tied to the JWT/credentials, not the profile, so it moved into auth-service as a whole with
 * Fase 7. The original VerificationController class was dead code (zero endpoints); this reuses
 * the name for the real thing.
 */
@RestController

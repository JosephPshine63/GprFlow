...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        if (user == null) {
            ...
            userServiceClient.createProfile(user.getId(), fullName, email);
        } else {
            // fullName is @Transient here — this instance was just loaded from the DB, so
            // it must be fetched from user-service before it can go into the JWT claims.
